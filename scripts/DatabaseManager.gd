...
func get_all_students() -> Array:
	_db.query("SELECT id, name, reading_level, xp, streak_days, onboarding_done FROM students ORDER BY name ASC")
...
func update_student_level(student_id: String, level: int) -> void:
	_db.query_with_bindings("UPDATE students SET reading_level = ? WHERE id = ?", [level, student_id])
...
func record_quest_attempt(student_id: String, quest_id: String, building_id: String,
						   passed: bool, score: int = 0, total_items: int = 0) -> void:
	var now := Time.get_datetime_string_from_system()
	_db.query_with_bindings(
