...
func _build_grass() -> void:
	...
	for _i in range(35):
		...
		for v in range(9):
			...
			pts.append(Vector2(cx + cos(a) * push * randf_range(1.1, 1.9),
	...
	noise.seed       = 77
	seed(55)
	var tuft_colors := [Color("#70c725"), Color("#5BAF20"), Color("#8ED44B"),
						Color("#4DB82A"), Color("#3D9815")]
	var placed := 0
	var tries  := 0
...
func _build_paths() -> void:
	...
	var vx0  := _vp.x * 0.46;  var vx1  := _vp.x * 0.54;  var vw  := vx1 - vx0
	var hy0  := _vp.y * 0.56;  var hy1  := _vp.y * 0.66;  var hh  := hy1 - hy0
	var sbx0 := _vp.x * 0.11;  var sbx1 := _vp.x * 0.19;  var sbw := sbx1 - sbx0
	var sbyo := _vp.y * 0.44;  var sbyb := hy0
	var lbx0 := _vp.x * 0.76;  var lbx1 := _vp.x * 0.84;  var lbw := lbx1 - lbx0
	var lbyo := _vp.y * 0.40;  var lbyb := hy0
	...
	_add_path_rect(Vector2(sbx0, sbyo), Vector2(sbw,   sbyb - sbyo),  c_edge, _path_shader, "#6B3A0A", "#8a5a10")
	_add_path_rect(Vector2(lbx0, lbyo), Vector2(lbw,  lbyb - lbyo),  c_edge, _path_shader, "#6B3A0A", "#8a5a10")
	...
	_add_path_rect(Vector2(vx0  + vw  * 0.10, 0),
				 Vector2(vw  * 0.80, _vp.y),          c_mid, _path_shader, "#8a5a10", "#c4811a")
	_add_path_rect(Vector2(0,    hy0  + hh  * 0.10),
				 Vector2(_vp.x,      hh  * 0.80),     c_mid, _path_shader, "#8a5a10", "#c4811a")
	_add_path_rect(Vector2(sbx0 + sbw * 0.10, sbyo),
				 Vector2(sbw * 0.80, sbyb - sbyo),    c_mid, _path_shader, "#8a5a10", "#c4811a")
	_add_path_rect(Vector2(lbx0 + lbw * 0.10, lbyo),
				 Vector2(lbw * 0.80, lbyb - lbyo),   c_mid, _path_shader, "#8a5a10", "#c4811a")

	# ── Pass 4: Center strips (40% of path width, lightest — most worn) ──
	_add_path_rect(Vector2(vx0  + vw  * 0.30, 0),
				 Vector2(vw  * 0.40, _vp.y),          c_center, _path_shader, "#c4811a", "#ecac22")
	_add_path_rect(Vector2(0,    hy0  + hh  * 0.30),
				 Vector2(_vp.x,      hh  * 0.40),     c_center, _path_shader, "#c4811a", "#ecac22")
	_add_path_rect(Vector2(sbx0 + sbw * 0.30, sbyo),
				 Vector2(sbw * 0.40, sbyb - sbyo),    c_center, _path_shader, "#c4811a", "#ecac22")
	_add_path_rect(Vector2(lbx0 + lbw * 0.30, lbyo),
				 Vector2(lbw * 0.40, lbyb - lbyo),   c_center, _path_shader, "#c4811a", "#ecac22")
	...
	var stone_colors := [Color("#7A6E65"), Color("#6E6358"), Color("#8A7D72"),
						 Color("#6A5E55"), Color("#9A8D80")]
	...
	pn.seed       = 44
	seed(123)
	var pcount := 0;  var pattempts := 0
	while pcount < 35 and pattempts < 500:
		pattempts += 1
		var px := 0.0;  var py := 0.0
		match randi() % 4:
			0: px = randf_range(vx0,  vx1);   py = randf() * _vp.y
			1: px = randf() * _vp.x;          py = randf_range(hy0, hy1)
			2: px = randf_range(sbx0, sbx1);  py = randf_range(sbyo, sbyb)
			_: px = randf_range(lbx0, lbx1);  py = randf_range(lbyo, lbyb)
...
func _make_top_tree(cx: float, cy: float, r: float, type: TreeType = TreeType.OAK) -> void:
	...
	var leaf_colors := [Color("#c87820"), Color("#d4943a"), Color("#b06818"),
						Color("#e8a848"), Color("#a05010")]
	for _i in range(randi_range(4, 6)):
		...
		var cr2 := cos(rot);  var sr2 := sin(rot)
		# Leaf shape (5-point, slightly curved)
		var lpts := PackedVector2Array()
		for p in [Vector2(0.0, -lr * 1.3), Vector2(lr * 0.7, -lr * 0.3),
				  Vector2(lr * 0.4, lr * 0.8), Vector2(-lr * 0.4, lr * 0.8),
				  Vector2(-lr * 0.7, -lr * 0.3)]:
			lpts.append(Vector2(p.x * cr2 - p.y * sr2 + lx,
								p.x * sr2 + p.y * cr2 + ly))
...
func _build_oak_foliage(root: Node2D, r: float) -> void:
	...
	var b0 := _make_foliage_circle(
		Vector2(r * 0.05, -r * 0.6), r * 1.4, dark_green)
	root.add_child(b0)

	# Main central blob
	var b1 := _make_foliage_circle(
		Vector2(-r * 0.85, -r * 0.95), r * 1.7, base_green)
	root.add_child(b1)

	# Upper-left cluster
	var b2 := _make_foliage_circle(
		Vector2(-r * 1.1, -r * 1.3), r * 1.2, mid_green)
	root.add_child(b2)

	# Right cluster
	var b3 := _make_foliage_circle(
		Vector2(r * 0.1, -r * 1.1), r * 1.1, mid_green)
	root.add_child(b3)

	# Top highlight blob
	var b4 := _make_foliage_circle(
		Vector2(-r * 0.6, -r * 1.4), r * 0.9, light_green)
	root.add_child(b4)

	# Small bright accent
	var b5 := _make_foliage_circle(
		Vector2(-r * 0.3, -r * 1.5), r * 0.5, Color("#8ED44B"))
	root.add_child(b5)
...
func _add_fruit_dots(root: Node2D, r: float) -> void:
	...
	var fruit_colors := [Color("#E03E3E"), Color("#FF6B6B"), Color("#FFD93D"),
						 Color("#FF8E53"), Color("#E03E3E")]
	for _i in range(randi_range(5, 8)):
		...
		var fruit := _make_circle_panel(
			Vector2(fx - fr, fy - fr), fr * 2.0,
			fruit_colors[randi() % fruit_colors.size()])
...
func _add_notice_board(pos: Vector2) -> void:
	...
	# Board frame
	_add_rect_child(root, Vector2(-18 * _sx, -52 * _sy), Vector2(36 * _sx, 2 * _sy), Color("#8B6914"))
	_add_rect_child(root, Vector2(-18 * _sx, -30 * _sy), Vector2(36 * _sx, 2 * _sy), Color("#8B6914"))
	_add_rect_child(root, Vector2(-18 * _sx, -52 * _sy), Vector2(2 * _sx, 24 * _sy), Color("#8B6914"))
	_add_rect_child(root, Vector2(16 * _sx, -52 * _sy), Vector2(2 * _sx, 24 * _sy), Color("#8B6914"))
	# Paper notes pinned to board
	_add_rect_child(root, Vector2(-12 * _sx, -48 * _sy), Vector2(10 * _sx, 12 * _sy), Color("#FFFEF5"))
	...
	# Pin dots
	for px in [Vector2(-7 * _sx, -49 * _sy), Vector2(6 * _sx, -47 * _sy), Vector2(-2 * _sx, -43 * _sy)]:
...
func _add_crates(pos: Vector2) -> void:
	...
	# Cross planks
	_add_rect_child(root, c1_pos + Vector2(0, 13 * _sy), Vector2(30 * _sx, 2 * _sy), Color("#5C3D11"))
	_add_rect_child(root, c1_pos + Vector2(14 * _sx, 0), Vector2(2 * _sx, 28 * _sy), Color("#5C3D11"))
	# Lid
	...
	_add_rect_child(root, c2_pos + Vector2(0, 12 * _sy), Vector2(26 * _sx, 2 * _sy), Color("#6B4B2A"))
	_add_rect_child(root, c2_pos + Vector2(12 * _sx, 0), Vector2(2 * _sx, 26 * _sy), Color("#6B4B2A"))
...
func _add_barrels(pos: Vector2) -> void:
	...
	for idx in bx_offsets.size():
		...
		var r := 14.0 * _sx
		# Main barrel body
		var barrel := _make_circle_panel(
			Vector2(bx - r, -r * 2.0 - r),
			r * 2.0, Color("#5C3D11"))
		...
		var top := _make_circle_panel(
			Vector2(bx - r * 0.7, -r * 2.9),
			r * 1.4, Color("#7A5522"))
		root.add_child(top)
	_ysort.add_child(root)
...
func _add_flowers(pos: Vector2) -> void:
	...
	var petal_colors := [Color("#FF6B6B"), Color("#FFD93D"), Color("#E8A0BF"),
...
func _add_lamp_post(pos: Vector2) -> void:
	...
	# Lamp head
	_add_rect_child(root, Vector2(-6 * _sx, -56 * _sy), Vector2(12 * _sx, 8 * _sy), Color("#6B6B73"))
	...
	var light := _make_circle_panel(
		Vector2(-4 * _sx, -54 * _sy), 8 * _sx, Color("#FFE4A0"))
...
func _add_fence_segment(pos: Vector2) -> void:
	...
	for i in range(3):
		...
		# Post cap
		_add_rect_child(root, Vector2(px - 1 * _sx, -26 * _sy), Vector2(6 * _sx, 3 * _sy), Color("#6B4B1A"))
	# 2 horizontal rails
...
func _add_stepping_stones(pos: Vector2) -> void:
	...
	root.position = pos
	var stone_offsets := [
		Vector2(-8 * _sx, -4 * _sy), Vector2(6 * _sx, -12 * _sy),
		Vector2(-4 * _sx, -20 * _sy), Vector2(8 * _sx, -28 * _sy),
	]
	for off in stone_offsets:
		var stone := Polygon2D.new()
		stone.polygon = _oval_pts(off.x, off.y,
			randf_range(5, 7) * _sx, randf_range(3, 5) * _sy, 8)
...
func _add_wagon(pos: Vector2) -> void:
	...
	root.add_child(shad)
	# Wagon body
	_add_rect_child(root, Vector2(-20 * _sx, -22 * _sy), Vector2(44 * _sx, 18 * _sy), Color("#8B5E3C"))
	# Side plank detail
	_add_rect_child(root, Vector2(-20 * _sx, -22 * _sy), Vector2(44 * _sx, 2 * _sy), Color("#5C3D11"))
	_add_rect_child(root, Vector2(-20 * _sx, -12 * _sy), Vector2(44 * _sx, 2 * _sy), Color("#5C3D11"))
	# Wheels (×2)
	...
	root.add_child(wrh)
	# Handle/pole
	_add_rect_child(root, Vector2(24 * _sx, -14 * _sy), Vector2(16 * _sx, 3 * _sy), Color("#5C3D11"))
	# Hay in wagon
...
func _add_mailbox(pos: Vector2) -> void:
	...
	root.position = pos
	# Post
	_add_rect_child(root, Vector2(-2 * _sx, -28 * _sy), Vector2(4 * _sx, 28 * _sy), Color("#5C3D11"))
	# Mailbox body
	_add_rect_child(root, Vector2(-8 * _sx, -38 * _sy), Vector2(16 * _sx, 12 * _sy), Color("#4A7ABF"))
	# Mailbox lid (slightly lighter)
	_add_rect_child(root, Vector2(-9 * _sx, -40 * _sy), Vector2(18 * _sx, 4 * _sy), Color("#5B8BD0"))
	# Flag
...
func _spawn_player() -> void:
	...
	var _gender: String = GameManager.current_student.get("character_gender", "male")
	var _sprite_path := "res://assets/sprites/character/player_female.png" if _gender == "female" else "res://assets/sprites/character/player.png"
	spr.texture  = load(_sprite_path)
	spr.name     = "Sprite"
	var ref_w    := 48.0 * _sx
	var ref_h    := 72.0 * _sy
	...
	if result.get("can_start", false):
		...
		if reason == "wrong_sequence":
			_quest_prompt.show_sequence_message(
				result.get("next_label", "the previous building"),
				controller.position,
				player
			)


func _on_quest_prompt_start(building_id: String) -> void:
	QuestManager.start_quest(building_id)
...
func _on_building_unlocked(building_id: String) -> void:
	print("[Main] Unlocked: ", building_id,
...
func _update_progress_bar() -> void:
	...
	if is_instance_valid(fill):
		var progress := GameManager.get_progress_percent() / 100.0
		var tw := create_tween()
		tw.tween_property(fill, "size:x", 180.0 * _sx * progress, 0.5) \
	...
	if is_instance_valid(count) and count is Label:
		...
		(count as Label).text = "%d / %d buildings restored" % [unlocked_count, BUILDING_DATA.size()]
...
func _start_tutorial() -> void:
	...
	_tutorial_overlay.tutorial_completed.connect(func() -> void:
...
func _add_rect_to(parent: Node, pos: Vector2, size: Vector2,
				  color: Color, z: int) -> ColorRect:
	var r := ColorRect.new()
	r.position = pos
	r.size     = size
	r.color    = color
...
func _add_path_rect(pos: Vector2, sz: Vector2, color: Color,
					shader: Shader, dark_hex: String, light_hex: String) -> void:
	var r := ColorRect.new()
	r.position = pos
	r.size     = sz
	r.color    = color
...
func _blade_pts(cx: float, cy: float, rot_deg: float, sf: float) -> PackedVector2Array:
	var base := [Vector2(0, 0), Vector2(-2.5, -3), Vector2(-1.8, -8),
				 Vector2(0, -14), Vector2(1.8, -8), Vector2(2.5, -3), Vector2(0, 0)]
	var pts := PackedVector2Array()
	var rad := deg_to_rad(rot_deg)
	var cr  := cos(rad);  var sr := sin(rad)
	for p in base:
		var bx: float = p.x * sf;  var by: float = p.y * sf
		pts.append(Vector2(bx * cr - by * sr + cx,
						   bx * sr + by * cr + cy))
	return pts

## Grass fringe tabs along a vertical path edge.
## inward=true → tab points in +x (right, into path from left edge).
func _path_fringe_v(edge_x: float, y_start: float, y_len: float,
					inward: bool, color: Color) -> void:
...
func _path_fringe_h(edge_y: float, x_start: float, x_len: float,
					inward: bool, color: Color) -> void:
